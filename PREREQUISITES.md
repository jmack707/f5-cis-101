...
- **Mirror images** into your local registry: the CIS image in `lab-vars.env`
  (`f5networks/k8s-bigip-ctlr:…`), `nginx/nginx-ingress:3.7.2` (Module 3), the
  hello-world demo image `nginxdemos/hello` (Modules 1–3), and the cafe demo image
  `nginxdemos/nginx-hello` (Module 4). Update the `image:` fields /
